...
await test('whitelabel terms, checkout reuse, strict ten-slot cap and onboarding fulfillment',async()=>{
 ...
 const body={...buyer,email:'white-first@example.com',package:'whitelabel',requestKey:crypto.randomUUID(),acceptedTerms:true,termsVersion:WHITELABEL.termsVersion};
 ...
 const more=await Promise.all(Array.from({length:11},(_,i)=>request('/api/order',{body:{...body,requestKey:crypto.randomUUID(),email:`white-${i}@example.com`}})));
